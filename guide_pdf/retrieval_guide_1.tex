...
\section{Start here: data science basics for retrieval}
\label{sec:start}

\workedbox{New to the topic?}{
...
\begin{itemize}
    \item \textbf{What the task is.} In ordinary classification, one input usually gets one label. In retrieval, one query is compared against many documents, and the output is an ordered list. So the goal is not just ``is this document good or bad?'' but ``which documents should appear first?''
    \item \textbf{Data science loop.} The loop is: define the question, collect the data, clean and normalize the text, turn text into numeric representations, choose a scoring model, rank documents, evaluate on held-out queries, inspect failures, and improve the pipeline. The notebook follows exactly this pattern.
    \item \textbf{The main dataset objects.} A \emph{document} belongs to the corpus we search through. A \emph{query} is the user need we want to satisfy. A \emph{sample} can mean one document row, one query row, or one query-document pair depending on the stage of the pipeline. A \emph{feature} is any numeric signal we compute from text. A \emph{label} tells us what the correct answer is; in retrieval, that usually means whether a document is relevant to a query.
    \item \textbf{Why train, validation, and test are separated.} We use \emph{train} data to fit parameters, \emph{validation} data to compare design choices such as \code{top_k} or preprocessing, and \emph{test} data only for the final unbiased check. Looking at test results too early causes \emph{data leakage}: the system starts to indirectly adapt to the test set instead of truly generalizing.
    \item \textbf{Text cleaning and normalization.} Raw text is messy: uppercase/lowercase differences, repeated spaces, punctuation variants, tags, ids, and separators like \code{-} or \code{/}. Normalization makes equivalent strings look more alike. In this notebook, the cleaning is intentionally light so that technical tokens such as version numbers, package names, and error codes are not destroyed.
    \item \textbf{Tokenization.} Tokenization breaks text into smaller units called \emph{tokens}. With simple tokenization, the sentence ``GPU-driver error 404'' can become tokens such as \code{gpu}, \code{driver}, \code{error}, and \code{404}. Once text is tokenized, the system can count tokens, weight them, or map them into embeddings.
    \item \textbf{Vectorization.} Computers do not compare raw paragraphs directly; they compare numbers. Vectorization maps a query or document to a numeric representation.
    \begin{itemize}
        \item \emph{Sparse lexical methods} represent exact token evidence. Bag-of-words, TF-IDF, and BM25+ all reward overlap in words or terms. They work especially well when exact wording matters, such as error messages, library names, commands, or product models.
        \item \emph{Dense semantic methods} represent meaning in a compact vector called an embedding. They can rank two texts near each other even when they use different surface words, which helps with paraphrases and vocabulary mismatch.
    \end{itemize}
    \item \textbf{Scoring and ranking.} After vectorization, the system computes a relevance score for every query-document pair. For TF-IDF or embeddings, that score is often cosine similarity or dot product. For BM25+, it comes from a lexical scoring formula that uses term frequency, document frequency, and document length. The exact numeric value matters less than the ordering it produces: higher score means the document should appear earlier in the ranking.
    \item \textbf{Top-k retrieval.} A retriever usually does not keep every scored document. It keeps only the best \(K\) documents, often written \code{top_k}. Small \(K\) is faster but may miss relevant documents; large \(K\) improves coverage but increases compute and can lower precision.
    \item \textbf{Evaluation.} We judge the system on held-out queries whose relevant documents are known. \emph{Recall@K} asks ``did we retrieve the relevant documents somewhere in the top \(K\)?'' \emph{Precision@K} asks ``how many of the top \(K\) were actually relevant?'' \emph{MAP} and \emph{MRR} care not only about whether relevant documents appear, but also how early they appear in the ranked list.
    \item \textbf{Mental model for the rest of the guide.} Whenever a later section introduces TF-IDF, BM25+, embeddings, or category filtering, ask three questions: what representation is being built, how are scores computed, and which evaluation metric improved? That simple checklist is enough to follow most of the notebook.
\end{itemize}

\purplebox{A tiny end-to-end example}{
Suppose the query is ``python socket timeout''. A lexical method such as TF-IDF or BM25+ will strongly favor a document containing the exact words \code{python}, \code{socket}, and \code{timeout}. A dense embedding model may also rank a document about ``network connection delays in Python'' even if it does not repeat all three words exactly.

So the full pipeline is:
\begin{enumerate}
    \item normalize and tokenize the query and documents,
    \item turn them into sparse or dense numeric representations,
    \item compute a score between the query and every candidate document,
    \item sort documents by score and check whether the truly relevant ones rise near the top.
\end{enumerate}

This is the same pattern repeated throughout the notebook; only the representation and scoring rule change.
}

\tipbox{If a later section feels abstract, come back to this page and identify the current step of the pipeline: cleaning, tokenization, vectorization, scoring, ranking, or evaluation. Once you know the step, the jargon becomes much easier to decode.}
...
\textbf{Lexical retrieval} rewards exact token overlap. It shines when the wording matters: error strings, API names, product models, flags, codes, and identifiers. \textbf{Semantic retrieval} uses learned embeddings to place related meanings near one another, so it succeeds when the query paraphrases the document instead of reusing the same words.
