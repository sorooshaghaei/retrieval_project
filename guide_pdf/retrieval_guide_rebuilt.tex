...
\hypersetup{
  ...
  linkcolor=TitleBlue,
  urlcolor=AccentTeal,
  pdftitle={Retrieval from Zero: A Beginner-First Study Manual},
  pdfauthor={OpenAI},
...
\node[anchor=north west,text width=0.78\paperwidth,align=left] at ([xshift=1in,yshift=-1.25in]current page.north west) {\color{white}\bfseries\fontsize{28}{32}\selectfont Retrieval from Zero\\[0.4em]A beginner-first study manual for understanding retrieval models, reranking, classification support, evaluation, and the full notebook pipeline};
\node[anchor=north west,text width=0.68\paperwidth,align=left] at ([xshift=1in,yshift=-4.45in]current page.north west) {\color{white}\Large This guide starts before information retrieval. It first teaches the minimum foundations in data science, text processing, vectors, and similarity so that later ideas such as TF--IDF, BM25, embeddings, bi-encoders, cross-encoders, category-aware reranking, validation, and leakage can be understood calmly rather than memorized mechanically.};
\node[anchor=south west,text width=0.75\paperwidth,align=left] at ([xshift=1in,yshift=1.1in]current page.south west) {\color{white}\large Designed as a self-contained learning path with worked examples, comparison tables, self-checks, and short answers.};
\end{tikzpicture}
\vfill
\end{titlepage}
...
\begin{multicols}{2}
\footnotesize
\textbf{How to use this manual}\dotfill i\\[0.2em]
\textbf{Part I. Before Retrieval: the beginner foundations}\dotfill 1\\
\hspace*{1em}1. What data science is, in plain language\dotfill 2\\
\hspace*{1em}2. Text as data\dotfill 5\\
\hspace*{1em}3. Vectors, matrices, and similarity from zero\dotfill 8\\[0.2em]
\textbf{Part II. Understanding the retrieval problem}\dotfill 11\\
\hspace*{1em}4. What information retrieval is\dotfill 12\\
\hspace*{1em}5. Lexical matching versus semantic matching\dotfill 15\\[0.2em]
\textbf{Part III. Lexical retrieval models}\dotfill 18\\
\hspace*{1em}6. Bag of words from zero\dotfill 19\\
\hspace*{1em}7. TF, DF, and IDF slowly and properly\dotfill 21\\
\hspace*{1em}8. TF--IDF for retrieval\dotfill 24\\
\hspace*{1em}9. BM25\dotfill 27\\
\hspace*{1em}10. BM25+\dotfill 30\\[0.2em]
\textbf{Part IV. Semantic retrieval models}\dotfill 32\\
\hspace*{1em}11. Embeddings from zero\dotfill 33\\
\hspace*{1em}12. Bi-encoder retrieval\dotfill 35\\
\hspace*{1em}13. Limits of embeddings\dotfill 37\\[0.2em]
\columnbreak
\textbf{Part V. Reranking and deeper relevance}\dotfill 39\\
\hspace*{1em}14. What a reranker is\dotfill 40\\
\hspace*{1em}15. Cross-encoder from zero\dotfill 42\\
\hspace*{1em}16. Hard negatives and training intuition\dotfill 44\\[0.2em]
\textbf{Part VI. Classification inside a retrieval pipeline}\dotfill 46\\
\hspace*{1em}17. What classification is\dotfill 47\\
\hspace*{1em}18. Why classification can help retrieval\dotfill 49\\
\hspace*{1em}19. TF--IDF + LinearSVC from zero\dotfill 51\\[0.2em]
\textbf{Part VII. Evaluation and experimental honesty}\dotfill 53\\
\hspace*{1em}20. Evaluation metrics from zero\dotfill 54\\
\hspace*{1em}21. Validation, holdout, and test\dotfill 57\\
\hspace*{1em}22. Hyperparameters and tuning\dotfill 60\\[0.2em]
\textbf{Part VIII. Engineering and notebook understanding}\dotfill 62\\
\hspace*{1em}23. Why engineering matters\dotfill 63\\
\hspace*{1em}24. Reading the notebook as a system\dotfill 65\\
\hspace*{1em}25. One-query journey through the whole pipeline\dotfill 68\\
\hspace*{1em}26. Full pipeline mental model\dotfill 71\\[0.2em]
\textbf{Progressive glossaries}\dotfill 74--81\\
\textbf{Appendix A. Final cheat sheet}\dotfill 82\\
\textbf{Master glossary}\dotfill 84\\
\end{multicols}

\clearpage
\pagenumbering{arabic}
\part{Before Retrieval: the beginner foundations}
\chapter{What data science is, in plain language}
...
\chapter{Vectors, matrices, and similarity from zero}
...
\part{Understanding the retrieval problem}
\chapter{What information retrieval is}
...
\chapter{Lexical matching versus semantic matching}
...
\part{Lexical retrieval models}
\chapter{Bag of words from zero}
...
\chapter{TF, DF, and IDF slowly and properly}
...
\part{Semantic retrieval models}
\chapter{Embeddings from zero}
...
\part{Reranking and deeper relevance}
\chapter{What a reranker is}
...
\chapter{Cross-encoder from zero}
...
\chapter{Hard negatives and training intuition}
...
\part{Classification inside a retrieval pipeline}
\chapter{What classification is}
...
\chapter{Why classification can help retrieval}
...
\chapter{TF--IDF + LinearSVC from zero}
...
\part{Evaluation and experimental honesty}
\chapter{Evaluation metrics from zero}
...
\chapter{Validation, holdout, and test}
...
\chapter{Hyperparameters and tuning}
...
\part{Engineering and notebook understanding}
\chapter{Why engineering matters}
...
\chapter{Reading the notebook as a system}
...
\chapter{One-query journey through the whole pipeline}
...
\chapter{Full pipeline mental model}
...
\begin{center}
\begin{tikzpicture}[scale=0.85,transform shape,node distance=0.6cm and 0.55cm, every node/.style={font=\small}]
