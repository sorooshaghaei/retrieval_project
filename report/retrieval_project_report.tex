...
\texttt{notebooks/kaggle-submission.ipynb}, which loads the competition data,
...
\node[box, fill=softrose]   (eval)   at (11.4,0)  {Evaluation\\MAP / Recall};
...
% ─────────────────────────────────────────────────────────────────────────────
\subsection{Method 4: Hybrid BM25+ $\to$ Embedding Re-ranking}
...
\paragraph{Mean Average Precision (MAP@K).}
For each query, Average Precision is computed over the top-$K$ retrieved
documents:
\[
\text{AP}(q) = \frac{1}{|\mathcal{R}_q|}
  \sum_{k=1}^{K} \text{Precision}(k) \cdot \mathbb{1}[d_k \in \mathcal{R}_q]
\]
...
\begin{tabular}{l c c}
\toprule
\textbf{Model} & \textbf{MAP@100} & \textbf{Recall@100} \\
\midrule
TF-IDF     & \textit{(from notebook)} & \textit{(from notebook)} \\
BM25+      & \textit{(from notebook)} & \textit{(from notebook)} \\
Embedding  & \textit{(from notebook)} & \textit{(from notebook)} \\
\textbf{Hybrid}  & \textit{(from notebook)} & \textit{(from notebook)} \\
\bottomrule
\end{tabular}
\end{center}

\studybox{Note}{Fill in the actual metric values after running the evaluation
cells in the notebook.  The \texttt{FINAL\_MODEL} parameter is then set to
...
\item \textbf{Evaluation.}  Run all four models on 327 training queries;
      compute MAP@100 and Recall@100.
