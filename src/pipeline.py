...
from src.models import run_bm25_search, run_dense_search, run_tfidf_search
...
FINAL_MODEL = "bm25"  # "bm25" or "tfidf" (or "dense" for slower final-only run)
RUN_DENSE_EVAL = False  # Set True to compare dense retrieval in phase-1 style evaluation.
DENSE_EVAL_QUERY_LIMIT = 100  # Dense model is expensive; evaluate on a subset by default.
...
def run_pipeline() -> None:
    ...
    if RUN_DENSE_EVAL:
        # Dense retrieval is much heavier, so evaluate on a configurable subset.
        dense_eval_queries = train_queries_df.head(DENSE_EVAL_QUERY_LIMIT).copy()
        dense_results = model_registry["dense"](docs_df, dense_eval_queries, top_k=EVAL_K)
        dense_qids = dense_eval_queries["id"].astype(str).tolist()
        dense_qrels_subset = {query_id: qrels.get(query_id, []) for query_id in dense_qids}
        dense_metrics = evaluate_retrieval(dense_results, dense_qrels_subset, k=EVAL_K)
        print(
            f"DENSE | "
            f"Precision@{EVAL_K}: {dense_metrics['avg_precision']:.4f} | "
            f"Recall@{EVAL_K}: {dense_metrics['avg_recall']:.4f} | "
            f"MRR@{EVAL_K}: {dense_metrics['mrr']:.4f} | "
            f"MAP@{EVAL_K}: {dense_metrics['map']:.4f} "
            f"(queries={len(dense_eval_queries)})"
        )
    else:
        print(
            "DENSE | Skipped in main pipeline (set RUN_DENSE_EVAL=True to include "
    ...
    if FINAL_MODEL == "dense" and FINAL_MODEL not in test_results_by_model:
        print("Running dense retrieval for final file (this may take a while)...")
        dense_results = model_registry["dense"](docs_df, test_queries_df, top_k=SUBMISSION_K)
        test_results_by_model["dense"] = dense_results

    if FINAL_MODEL not in test_results_by_model:
        raise ValueError(
            f"FINAL_MODEL='{FINAL_MODEL}' is not available. "
            f"Choose one of: {sorted(test_results_by_model)}"
        )
