...
\begin{tikzpicture}[x=1cm,y=1cm,>=Latex]
  ...
  \textbf{Report objective.} This version updates the written report to precisely match our experimental progression and the current saved notebook. We detail our journey from TF-IDF and BM25+ baselines to a robust dense embedding retriever. The active configuration uses embedding first-stage retrieval, cross-encoder second-stage reranking, and a TF-IDF + LinearSVC category classifier used as a soft category prior.\\[0.25cm]
...
Submission size target & Top 7,500 first-stage candidates; rerank top 30 with cross-encoder \\
Date & \today \\
\end{tabular}
...
The current system relies on a highly optimized dense first-stage retriever followed by a cross-encoder reranker, with category information injected as a soft bonus.
...
Cross-encoder reranker & Yes & Yes & Re-ranks the first-stage candidates (top-\(m\)) using joint query-document scoring. \\
Category classifier & Yes & Yes & Predicts one category per query and provides a soft category bonus during reranking. \\
...
\subsection{Phase 5: Hyperparameter Tuning and Two-Stage Reranking}
...
To refine this massive list, we trained a TF-IDF + LinearSVC category classifier and added a cross-encoder reranker. The reranker now rescores the top-\(m\) candidates (\texttt{top\_m=30}), while the predicted category is used as a soft bonus rather than a strict hard filter. This keeps recall robust while improving top-rank ordering.

\section{Evaluation Protocol and Results}

The notebook evaluates the optimized embedding run against the 327 training queries after optional second-stage reranking and category-aware score boosting. The combined score is calculated as:
\[
...
\midrule
Embedding & 7,500 & 0.94762 & 0.00110 & 0.63738 & 0.92661 & 0.62818 \\
\bottomrule
\end{tabular}
\caption{Offline metrics reported by the saved notebook execution snapshot.}
\end{table}

These results show strong recall and a substantial MRR gain after introducing second-stage reranking. Since the notebook configuration was later synchronized to use \texttt{CROSS\_ENCODER\_RERANK\_TOP\_M = 30} consistently for both evaluation and submission, rerunning the notebook can produce slightly different final numbers.

\section{Conclusion}

Our system evolved from basic lexical matching to a highly optimized two-stage semantic retrieval pipeline. By implementing dense embeddings, we solved the semantic mismatch problem; by engineering a robust caching and chunking system, we solved the 1.5-hour computational bottleneck; and by adding cross-encoder reranking with category-aware soft priors, we improved ranking quality at the top of the list. This architecture represents the current SeaFour submission pipeline.
